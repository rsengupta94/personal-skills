---
name: write-readme
description: Write or update a public repo's README — a brief problem-led intro plus run instructions verified against the repo's actual files. Use when the user wants a README created or refreshed, or says "write a readme", "update the readme", or "readme this repo".
---
...
- CLAUDE.md and project docs — for intent and problem framing only
...
**Gate — grounding:** if the problem the repo solves cannot be grounded in its code, docs, or the current session, ask the user for it. Never invent the why.
...
1. **Title + one line** — what it is. No adjectives.
2. **Problem** — 2–4 sentences, hard cap: the situation, its cost, why existing options fall short. The only persuasion in the document; it persuades through specificity.
3. **What it does** — concrete capabilities, one usage example.
4. **Quickstart** — prerequisites with versions, install, run, expected result. Copy-pasteable.
5. **Configuration** — only variables the code reads. Placeholder values.
6. **How it works** — one paragraph, only when the design is non-obvious. This section serves the evaluating reader: decisions and why, not narration.
7. **Status / license.**
